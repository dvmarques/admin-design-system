...
- Sincronizar obrigatoriamente a mesma branch `release/X.Y.Z` de volta para `develop` por PR após o merge em `master`, mantendo-a congelada para novas mudanças funcionais até o retorno terminar.
- Exigir que o PR de retorno para `develop` contenha somente o estado de preparação já integrado em `master` e, quando necessário, ajustes estritamente de resolução de conflitos desse retorno.
