...
test('publication blocks before writes when more than one back-merge matches', async () => {
	const requests = [];
	const pull = (sha) => ({
		merged_at: '2026-09-08T00:00:00Z',
		head: { repo: { full_name: 'org/repo' }, ref: 'release/1.0.0' },
	...
	const server = createServer((request, response) => {
		...
		response.end(
			JSON.stringify(
				request.url?.includes('base=master')
					? [{ ...pull('release-sha'), base: { ref: 'master' } }]
					: [pull('backmerge-a'), pull('backmerge-b')],
			),
		);
	});
	server.listen(0, '127.0.0.1');
	...
	server.close();
	assert.notEqual(result.code, 0);
	assert.match(result.stderr, /encontradas 2/);
	assert.equal(
		requests.some(({ method }) => method !== 'GET'),
		false,
	);
