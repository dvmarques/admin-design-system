...
- [ ] 1.4 Validar unicidade e ordem SemVer decrescente das versões fechadas do changelog.
- [ ] 1.5 Após o bootstrap, validar que a versão coordenada atual corresponde à última versão fechada e que o alvo é SemVer estritamente superior.
- [ ] 1.6 Validar branch `release/X.Y.Z`, working tree limpa, exatamente uma seção `Em andamento`, ausência de conflito e consistência das versões atuais.
- [ ] 1.7 Permitir renomear a única seção `Em andamento` do placeholder para patch/minor/major escolhido, preservando conteúdo.
- [ ] 1.8 Fechar a seção escolhida usando data `dd-mmm-aaaa` em português e criar acima o próximo patch apenas como placeholder.
- [ ] 1.9 Atualizar de forma coordenada raiz, `packages/*`, `apps/*` e referências internas para `X.Y.Z`.
- [ ] 1.10 Adicionar `packageManager` com versão exata do npm ao projeto e garantir uso da mesma versão localmente e na CI.
- [ ] 1.11 Regenerar `package-lock.json` na staging sem atualizar dependências externas e validar o resultado com instalação reproduzível.
- [ ] 1.12 Aplicar as mudanças reais somente após staging completa; implementar rollback/restauração integral ou substituição atômica equivalente.
- [ ] 1.13 Expor o comando como `npm run release:prepare -- X.Y.Z`.
...
- [ ] 2.3 Reutilizar a mesma lógica de parsing para determinar maior versão fechada, detectar duplicidades e validar ordem SemVer decrescente.
...
- [ ] 3.7 Nas releases seguintes, validar que a última versão fechada anterior possui tag anotada e GitHub Release consistentes antes de permitir merge.
...
- [ ] 5.4 Garantir que o PR de retorno contenha somente o estado já integrado em `master` e ajustes estritamente necessários para resolução de conflitos do back-merge.
- [ ] 5.5 Garantir resolução explícita de conflitos com mudanças posteriores em `develop` e proibir force update de refs.
- [ ] 5.6 Na primeira implantação, garantir que `release.yml` chegue à default branch `develop` antes do primeiro `workflow_dispatch` quando ainda não existir nela.
- [ ] 5.7 Documentar/validar que a branch de release só pode ser removida após o retorno para `develop`.
- [ ] 5.8 Documentar/validar que uma nova release não deve começar enquanto `develop` não contiver o estado pós-release anterior.

## 6. Testes

- [ ] 6.1 Testar bootstrap com alvo igual à versão coordenada atual e com alvo maior; rejeitar downgrade.
- [ ] 6.2 Testar determinação da maior SemVer fechada independentemente da posição textual e rejeitar versões fechadas duplicadas ou fora de ordem.
...
- [ ] 6.13 Testar bootstrap sem publicação anterior e bloqueio da próxima release quando a anterior não estiver publicada consistentemente.
...
- [ ] 6.20 Testar/validar o fluxo `release/X.Y.Z -> develop`, incluindo rejeição de nova mudança funcional pós-merge e preservação de mudanças concorrentes em `develop`.

## 7. Documentação

- [ ] 7.1 Criar `docs/release-process.md` com bootstrap, definição da última versão fechada, escolha de versão, preparação transacional, toolchain, CI, proteção exclusiva de `master`, serialização, publicação e recuperação.
...
- [ ] 7.6 Documentar congelamento da release branch após merge em `master` e escopo permitido no PR de retorno para `develop`.
...
- [ ] 8.3 Revisar fluxo de bootstrap `0.0.1`, publicação, retorno congelado para `develop` e release seguinte sem efetuar publicação real indevida.
- [ ] 8.4 Revisar cenários de falha/recuperação: atomicidade local, changelog inválido, push direto, release anterior ausente, publicação concorrente, commit não alcançável, lightweight tag e release preexistente divergente.
