...
test('preserva semântica e acesso aos dados em viewport estreita', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { level: 1, name: 'Apresentação de dados administrativos' })).toBeVisible();
...
test('expõe progresso e estado vazio sem semântica de erro implícita', async ({ page }) => {
	...
	await expect(page.getByRole('progressbar', { name: 'Importação' })).toHaveAttribute('aria-valuenow', '65');
	await expect(page.getByRole('progressbar', { name: 'Processando' })).not.toHaveAttribute('aria-valuenow');
