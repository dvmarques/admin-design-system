...
export function ThemeToggle({ className, initialTheme = 'light', onClick, onThemeChange, ...props }: ThemeToggleProps) {
	...
	return (
		<button
			...
			className={classNames('rounded-md border border-border bg-surface px-3 py-2 text-text', className)}
			onClick={(event) => {
				onClick?.(event);
