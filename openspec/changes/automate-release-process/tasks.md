...
- [ ] 1.3 Rejeitar prerelease/build metadata e validar bootstrap, downgrade e ordenação da versão alvo.
- [ ] 1.4 Validar uma única seção `Em andamento`, unicidade/ordem SemVer decrescente das versões fechadas e ausência de conflito com o alvo.
- [ ] 1.5 Permitir renomear o placeholder `Em andamento` para patch/minor/major escolhido, preservando conteúdo.
- [ ] 1.6 Fechar a versão usando data civil em `America/Sao_Paulo`, formato `dd-mmm-aaaa` e abreviações PT-BR fixas.
- [ ] 1.7 Criar acima o próximo patch apenas como placeholder `Em andamento`.
- [ ] 1.8 Atualizar de forma coordenada raiz, `packages/*`, `apps/*` e referências internas versionadas para `X.Y.Z`.
- [ ] 1.9 Fixar uma versão exata do npm em `packageManager` e garantir uso da mesma versão na preparação e na CI.
- [ ] 1.10 Regenerar `package-lock.json` sem atualizar dependências externas como efeito colateral e validar o resultado com instalação reproduzível.
- [ ] 1.11 Implementar rollback/restauração ou substituição atômica equivalente para impedir estado parcial em falha de aplicação.
- [ ] 1.12 Expor o comando como `npm run release:prepare -- X.Y.Z`.
...
- [ ] 2.4 Validar publicação consistente da predecessora quando ela existir.
...
- [ ] 4.4 Serializar publicações com um único grupo de concorrência, preservando execuções pendentes e sem cancelar publicação em andamento.
- [ ] 4.5 Resolver de forma inequívoca a PR merged `release/X.Y.Z -> master` e obter o commit efetivamente integrado.
...
- [ ] 4.10 Criar GitHub Release `vX.Y.Z` não-draft/não-prerelease usando somente notas extraídas do changelog.
...
- [ ] 6.2 Testar bootstrap com alvo igual/maior à versão atual e rejeição de downgrade.
- [ ] 6.3 Testar maior SemVer fechada, predecessora, duplicidade, ordem inválida e estados inválidos de `Em andamento`.
- [ ] 6.4 Testar renomeação do placeholder para patch/minor/major preservando conteúdo.
- [ ] 6.5 Testar data em `America/Sao_Paulo`, inclusive processo em outra timezone e fronteira de mudança de dia.
- [ ] 6.6 Testar falhas de preflight, staging, lockfile e aplicação final sem estado parcial.
- [ ] 6.7 Testar versionamento coordenado e ausência de atualização externa não relacionada no lockfile.
- [ ] 6.8 Testar extração e comparação normalizada das release notes.
- [ ] 6.9 Testar validação da predecessora ausente, consistente e divergente.
- [ ] 6.10 Testar política de PR para `master` e comportamento distinto em `push` pós-merge.
- [ ] 6.11 Testar resolução do commit da release com `master` avançado e falha para commit não alcançável/ambíguo.
- [ ] 6.12 Testar tag inexistente, anotada correta, lightweight e anotada em outro commit.
- [ ] 6.13 Testar recuperação tag válida + release ausente.
- [ ] 6.14 Testar release existente consistente e divergências em nome, draft, prerelease, tag, commit ou body.
- [ ] 6.15 Validar serialização do workflow e rejeição operacional de dispatch em ref diferente de `develop`.
- [ ] 6.16 Testar back-merge com novas entradas de changelog após o corte.
- [ ] 6.17 Testar back-merge com workspace novo após o corte, preservando metadados futuros.
- [ ] 6.18 Testar rejeição de delta funcional/arquivo fora do escopo permitido no PR de retorno.
...
- [ ] 7.6 Documentar política de `master`, required checks/ruleset e comportamento distinto entre `pull_request` e `push`.
- [ ] 7.7 Documentar back-merge, bloco fechado imutável, workspaces novos e retenção da branch.
- [ ] 7.8 Documentar resolução do commit, tags anotadas, GitHub Release e recuperação idempotente.
- [ ] 7.9 Adicionar ao `README.md` resumo do processo e link para `docs/release-process.md`.
- [ ] 7.10 Adicionar em `AGENTS.md` referência operacional curta para agentes, apontando para `docs/release-process.md` sem duplicar o procedimento.
- [ ] 7.11 Revisar `openspec/config.yaml`; adicionar apenas orientação release-specific que seja útil a futuras changes e não duplique a documentação. Registrar no PR se nenhuma mudança for necessária.
